...
export class shoppingListService {
  ...
  addRecipe(recipeId: number, servings: number, name: string, ingredients: RecipeIngredient[]) {
    ...
    this.recipes.update((list) => [...list, { uid, recipeId, servings, name, exist: true }]);
  ...
  updateServings(uid: number, newServings: number) {
    ...
    this.recipes.update((list) =>
      list.map((r) => (r.uid === uid ? { ...r, servings: newServings } : r)),
    );
  ...
  createShoppingList(request: CreateShoppingRequest) {
    return this.http.post<ShoppingListResponse>(`${this.apiUrl}`, request);
  }
  ...
  getSavedListById(id: number) {
    return this.http.get<ShoppingListResponse>(`${this.apiUrl}/${id}`);
  }
